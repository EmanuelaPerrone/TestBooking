...
<head>
  ...
  <title>Test Booking - Esiti tamponi Medico</title>
...
<body id="myPage" data-spy="scroll" data-target=".navbar" data-offset="60">
        ...
        <div class="row">
          <div class="banner-info" style="padding-top: 100px;">
            ...
            <div class="banner-text text-center">
              <h1 class="white">ESITI TAMPONI COVID-19<br>MEDICO</h1>
            </div>
  ...
  <section id="testimonial" class="section-padding">
    <div class="container">
      <div class="row">
        <div class="col-md-12">
          <h2 class="ser-title">Esiti tamponi COVID-19 - Medico</h2>
          <hr class="botm-line">
        </div>
  ...
  $conn = new mysqli($host, $user, $pass, $db);
	if ($conn->connect_error) {
      die("Connessione al database fallita: " . $conn->connect_error);
		...
		// Recupero i valori
        $cf = $_POST['codice_fiscale1'];
		$ce = $_POST['codice_esiti1'];
	
    $query = "SELECT codice_fiscale,nome,cognome,esito FROM pazienti WHERE id_medico = '$cf'";
	$result = $conn->query($query);
	
    echo "<center><h3>Le prenotazioni saranno visualizzate dopo aver effettuato il tampone<br>Prenotazioni con codice <font face='Arial'> $ce</font>:<br><br></h3></center>";

    if ($result->num_rows > 0) {
     while($row = $result->fetch_assoc()) {
     echo '<center><p style="color:black; font-size: 130%">➤ <strong>Codice fiscale: </strong>' .$row["codice_fiscale"].'<br><br><strong> Nome: </strong>' .$row["nome"].'<br><br><strong> Cognome: </strong>' .$row["cognome"].'<br><br><strong>Esito: </strong>' .$row["esito"].'</p></center><br>';  	
     }	
	}
	 else {
        echo "<font face='Open Sans'><h3><em class='fas fa-times-circle'></em> Nessuna corrispondenza trovata. Controlla i campi e riprova.</h3></font><br><center><button type='button' onclick='javascript:history.back()' class='btn btn-primary'>Indietro</button></center>";
     } 
		 mysqli_close($conn);
		 
?>
<div align="center">
   <br><br><a href="index.php" class="btn btn-primary">Home</a>
  </div>
      </div>
    </div>
  </section>
