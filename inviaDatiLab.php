...
<?php
 ...
 $db = "my_testbooking";
  ...
  $conn = new mysqli($host, $user, $pass, $db);
    ...
    $query = "SELECT nome FROM laboratori WHERE codice_univoco = '$codice_univoco'";
	$result = $conn->query($query);
	
    if ($result->num_rows > 0) {
     while($row = $result->fetch_assoc()) {
       echo "<h3>Accesso effettuato come: " . $row["nome"]. "</h3><br><center><a href='visualizzaPren.php?codice_univoco=$codice_univoco' class='btn btn-primary btn-lg btn-block'>Visualizza prenotazioni</a><a href='aggiornaDate.php' class='btn btn-primary btn-lg btn-block'>Aggiorna disponibilità</a><a href='datiEnti.php' class='btn btn-primary btn-lg btn-block'>Comunica esiti agli organi competenti</a><a href='datiSistema.php' class='btn btn-primary btn-lg btn-block'>Comunica esiti agli utenti</a></center>";	
	 }
	}
	else {
	  echo "<h3 style='font-family: Open Sans'>Nessuna corrispondenza trovata. Controlla i campi e riprova.</h3><br><center><button type='button' onclick='javascript:history.back()' class='btn btn-primary'>Indietro</button></center>";				 
	}
